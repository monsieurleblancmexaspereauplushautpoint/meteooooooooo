...
angular.module('starter', ['ionic', 'starter.controllers'/*,'textAngular'*/])
  ...
  .controller('MainCtrl', function ($scope) {
    ...
    $scope.tamerelachaudiere = function() {
      ...
      //var minTemp, timeMinTemp, minHumi, timeMinHumi, maxTemp, timeMaxTemp, maxHumi, timeMaxHumi, avgTemp, avgHumi, countTemp, countHumi;
      var tab = $scope.content.split('\n');
      var computedData;
      // tab[x][y]    ===> x : temp(0), humi(1), pres(2), wspeed(3), wdir(4)
                    //===> y : min(0), max(1), timeMin(2), timeMax(3), count(4)
          computedData =  [
             [parseFloat(tab[2].split("\t")[33]),parseFloat(tab[2].split("\t")[33]), tab[2].split("\t")[0] + " " + tab[2].split("\t")[1], tab[2].split("\t")[0] + " " + tab[2].split("\t")[1], 0]
            ,[parseFloat(tab[2].split("\t")[35]),parseFloat(tab[2].split("\t")[35]), tab[2].split("\t")[0] + " " + tab[2].split("\t")[1], tab[2].split("\t")[0] + " " + tab[2].split("\t")[1], 0]
            ,[parseFloat(tab[2].split("\t")[38]),parseFloat(tab[2].split("\t")[38]), tab[2].split("\t")[0] + " " + tab[2].split("\t")[1], tab[2].split("\t")[0] + " " + tab[2].split("\t")[1], 0]
            ,[parseFloat(tab[2].split("\t")[3]), parseFloat(tab[2].split("\t")[3]), tab[2].split("\t")[0] + " " + tab[2].split("\t")[1], tab[2].split("\t")[0] + " " + tab[2].split("\t")[1], 0]
            ,[parseFloat(tab[2].split("\t")[9]), parseFloat(tab[2].split("\t")[9]), tab[2].split("\t")[0] + " " + tab[2].split("\t")[1], tab[2].split("\t")[0] + " " + tab[2].split("\t")[1], 0]];
